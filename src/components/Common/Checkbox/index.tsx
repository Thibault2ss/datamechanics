...
const CheckboxLabel = styled.label`
  ...
  input:checked ~ ${CheckboxCustom} {
    background-color: ${props => props.color ? props.color : variables.greenColor};
    border-radius: 5px;
    opacity: 1;
    border: 2px solid ${props => props.color ? props.color : variables.greenColor};
  }
...
const Checkbox: React.FC<IContentTitleProps> = props => {
  const { name, value, checked, color, handleCheckbox } = props
